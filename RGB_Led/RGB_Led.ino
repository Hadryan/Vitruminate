...
void setup() {
  // start taking in values from usart
  Serial.begin(9600);
  ...
  switch(mode) {
    case 0:
      ...
      if (Serial.available() > 0) {
        // make sure we are receiving values over usart
...
/**Fucntion to check if the lamp should either enter a HIGH mode,
...
void check_high(int avg) {
  ...
  if (high.times > 20 && millis() - high.times_start < 90.0)
    songmode = HIGH;
...
void visualize_music() {
  ...
  // Actual sensor value
  sensor_value = incomingByte; //analogRead(ANALOG_READ);
